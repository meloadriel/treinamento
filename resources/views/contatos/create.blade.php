...
<body class="flex flex-col items-center justify-center min-h-screen py-12 bg-[#ffefcc]">
    ...
    ">
        ...
        <form action="{{ route('contatos.store') }}" method="POST" class="h-full bg-zinc-100">
            @csrf

            <div class="flex items-center justify-between w-full px-6 py-4 bg-zinc-200 ">
                <a href="{{ route('contatos.index') }}" class="text-sm text-blue-500">
                    Cancelar
                </a>
                <h1 class="text-lg font-bold">Novo Contato</h1>
                <button type="submit" class="px-2 text-sm text-white rounded-full hover:text-blue-300">OK</button>
            </div>
            <div class="px-2 bg-white">
                <input type="text" id="nome" name="nome" class="w-full px-2 py-1 text-sm" required
                    placeholder="Digite aqui seu nome">

                <input type="text" id="cidade" name="cidade" class="w-full px-2 py-1 text-sm border-t " required
                    placeholder= "Digite o nome da sua cidade">

                <input type="text" id="rua" name="rua" class="w-full px-2 py-1 text-sm border-t " required
                    placeholder= "Digite o nome da sua Rua">

                <input type="text" id="numero" name="numero" class="w-full px-2 py-1 text-sm border-t " required
